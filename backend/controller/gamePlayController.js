...
const findQuestionsForLevel = async (game, level) => {
  ...
  if (levelQuestions.length === 0 && game.questionIds && game.questionIds.length > 0) {
...
const processFiftyFifty = (currentQuestion) => {
  ...
  const fiftyFiftyOptions = ['A', 'B', 'C', 'D'].filter((_, i) => 
    i === correctIndex || i === keepWrongIndex
  );
...
const processAskAudience = (currentQuestion) => {
  ...
  const wrongLetters = letters.filter(letter => letter !== currentQuestion.correctAnswer);
...
export const submitAnswer = async (req, res) => {
  try {
    ...
    const currentQuestion = await findQuestion(game, gameSession.currentQuestion);
    ...
    if (isCorrect) {
      if (gameSession.currentLevel === (game.maxLevel || 15) || gameSession.currentLevel === 15) {
